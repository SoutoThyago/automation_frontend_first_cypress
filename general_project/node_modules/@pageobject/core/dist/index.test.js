...
const _1 = require(".");
class JSDOMPage {
    async findElements(selector, parent) {
        return Array.from((parent || document.documentElement).querySelectorAll(selector));
    }
}
class Root extends _1.PageObject {
    constructor() {
        super(...arguments);
        this.selector = 'div';
    }
    async getName() {
        return (await this.getElement()).getAttribute(`${this.constructor.name}-name`);
    }
}
...
describe('Root', () => {
    ...
    describe('where()', () => {
        ...
        it('should throw a selection-criterion-already-defined error', () => {
            expect(() => root.where(jest.fn()).where(jest.fn())).toThrow('A selection criterion is already defined');
        });
    });
    describe('getElement()', () => {
        it('should fail to get the element of root', async () => {
            await expect(root.getName()).rejects.toThrow('DOM element not unique (Root)');
        });
        ...
        it('should fail to get the element of root3', async () => {
            await expect(root3.getName()).rejects.toThrow('DOM element not found (Root)');
        });
    });
    describe('getSize()', () => {
...
describe('Child', () => {
    ...
    describe('getElement()', () => {
        it('should fail to get the element of child', async () => {
            await expect(child.getName()).rejects.toThrow('DOM element not unique (Child)');
        });
        ...
        it('should fail to get the element of child3', async () => {
            await expect(child3.getName()).rejects.toThrow('DOM element not found (Child)');
        });
    });
    describe('getSize()', () => {
...
describe('Grandchild', () => {
    describe('getElement()', () => {
        it('should fail to get the element of grandchild', async () => {
            await expect(grandchild.getName()).rejects.toThrow('DOM element not unique (Grandchild)');
        });
        ...
        it('should fail to get the element of grandchild3', async () => {
            await expect(grandchild3.getName()).rejects.toThrow('DOM element not found (Grandchild)');
        });
    });
    describe('getSize()', () => {
